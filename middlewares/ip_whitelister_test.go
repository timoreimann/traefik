	...
	for index, e := range cases {
		t.Run(index, func(t *testing.T) {
			...
			for index, actual := range whitelister.whitelists {
				expeced := e.expectedWhitelists[index]
				if (!actual.IP.Equal(expeced.IP) || actual.Mask.String() != expeced.Mask.String()) {
					t.Errorf("unexpected result while comparing parsed ip whitelists, expected %s, got %s", actual, expeced)
				}
			}
		})
	}
